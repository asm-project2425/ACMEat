...
Interactive Swagger UI is available at:  **[http://localhost:8080/swagger-ui/index.html](http://localhost:8080/swagger-ui/index.html)**
...
* **Method**: `GET`
* **URL**: `/api/v1/bank/payment?orderId=...`
* **Query Parameters**:

  * `orderId`: ID of the order to complete payment for
* **Description**: Returns a URL for the user to complete the payment via the external banking service.
* **Response**: JSON object containing the URL to redirect the user.

#### `verifyPayment`

* **Method**: `POST`
* **URL**: `/api/v1/bank/verify-payment`
* **Request Body**:

```json
{
  "orderId": 42,
  "paymentToken": "abc-def-123"
}
```
...
* **Description**: Sends the `orderDelivered` message to the Camunda process to indicate that the order has been successfully delivered.
* **Response**: HTTP `204 No Content`.
